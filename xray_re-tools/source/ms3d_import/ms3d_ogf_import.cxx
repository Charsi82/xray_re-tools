...
int ms3d_ogf_import::import(const xr_ogf *ogf)
{
	...
	for (std::vector<xray_re::xr_bone*>::const_iterator it = sf.begin(), end = sf.end(); it != end; ++it) {
		std::string bname = (*it)->name();
		int ind = msModel_FindBoneByName(m_model, bname.c_str());
		msBone* bone = msModel_GetBoneAt(m_model, ind);
		std::string mtl = "gamemtl = ";
		msBone_SetComment(bone, mtl.append((*it)->gamemtl()).c_str());
	}

	msModel_SetComment(m_model, ogf->userdata().c_str());
...
int ms3d_ogf_import::import(const xr_ogf* ogf, const int* bone_refs)
{
	...
	std::string comment = "texture = ";
	comment.append(ogf->texture());
	msMaterial_SetComment(mtl, comment.append("\r\nshader = ").append(ogf->shader()).c_str());
...
//msg("vert%d: sum=%d, infls=%d", vert_idx, sum, num_xinfls);
	...
	for (size_t i = 0, num_inds = ib.size(); i != num_inds;) {
		...
		//word nIndices[3] = vb.n(v0);
		//msTriangle_SetNormalIndices(tri, nIndices);

// 			int nNormal0 = msMesh_AddVertexNormal(mesh);
// 			msMesh_SetVertexNormalAt(mesh, nNormal0, n.set(vb.n(v0)).xyz);
// 			int nNormal1 = msMesh_AddVertexNormal(mesh);
// 			msMesh_SetVertexNormalAt(mesh, nNormal1, n.set(vb.n(v1)).xyz);
// 			int nNormal2 = msMesh_AddVertexNormal(mesh);
// 			msMesh_SetVertexNormalAt(mesh, nNormal2, n.set(vb.n(v2)).xyz);
// 			word nNormalIndices[3] = { nNormal0, nNormal1, nNormal2 };
// 			msTriangle_SetNormalIndices(tri, nNormalIndices);
		}
	}
	delete[] vert_refs;
