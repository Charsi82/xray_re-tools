...
const char* ms3d_xray_export::GetTitle() { return "S.T.A.L.K.E.R... (*.object, *.skl"/*__DATE__*/")"; }
...
//	ofn.FlagsEx = 0;
	...
	if ( status < 0 ){
		TCHAR* new_ext = (ofn.nFilterIndex == 1) ? _T(".object") : _T(".skl");
		strcat_s(path, new_ext);
		msg("Extension not found! Try export to %s", path);
// 		TCHAR txtmsg[MAX_PATH] = _T("Расширение не указано. Экспортируем в ");
// 		strcat_s(txtmsg, fn);
// 		strcat_s(txtmsg, new_ext);
// 		strcat_s(txtmsg, _T("!"));
// 		MessageBox(NULL, txtmsg, _T("X-Ray Exporter"), MB_OK|MB_ICONWARNING);
		ms3d_object_export object_io(model);
		status = object_io.do_export(path);
	}	
