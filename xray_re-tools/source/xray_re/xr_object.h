...
#else
struct hash_value {
	size_t operator()(const xr_raw_surface& surface) const
	{
#if SIZE_MAX == _UI64_MAX
		return surface.blob64;
#else
...
public:
	...
	void set_userdata(std::string& ud){ m_userdata = ud; };//+
	...
	void reset_bones_parts(xr_partition_vec&) const;
...
#else
	std::unordered_map<xr_raw_surface, xr_surface*, hash_value>
#endif
#else
