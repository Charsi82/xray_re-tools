...
template<> inline void xr_ini_packet::write_number<uint64_t>(const uint64_t& value) {
	int n = xr_snprintf(m_key_buffer, sizeof(m_key_buffer), "%06d", ++m_counter);
	n = xr_snprintf(m_temp_buffer, sizeof(m_temp_buffer), "%lld", value);
	w->write(m_key_buffer, m_temp_buffer, false);
	w_seek(w_tell() + sizeof(uint64_t));
}
